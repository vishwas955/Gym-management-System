...
const ManageMemberships = () => {
    ...
    const fetchMemberships = async () => {
        try {
            ...
            setFilteredMemberships(response.data.memberships);
    ...
    const renderHeader = (doc, pageWidth, margin) => {
        ...
        doc.setFontSize(14);
        doc.text("Membership Report", 50, 35);
    ...
    const generatePDFReport = () => {
        ...
        if (!filteredMemberships.length) {
            alert("No memberships available to generate report!");
            return;
        }
        ...
        const tableColumn = ["Member", "Subscription", "Start Date", "End Date", "Status"];
        const tableRows = filteredMemberships.map(membership => [
            `${membership.gymMemberId?.first_name || ''} ${membership.gymMemberId?.last_name || ''}`,
        ...
        doc.save("caliber_fitness_membership_report.pdf");
    };
    ...
    const generateExcelReport = () => {
        ...
        const data = filteredMemberships.map(membership => ({
            Member: `${membership.gymMemberId?.first_name || ''} ${membership.gymMemberId?.last_name || ''}`,
            Subscription: membership.subscriptionId?.name || 'N/A',
            'Start Date': membership.startDate ? new Date(membership.startDate).toLocaleDateString() : 'N/A',
            'End Date': membership.endDate ? new Date(membership.endDate).toLocaleDateString() : 'N/A',
        ...
        workbook.Props.Title = "CALIBER FITNESS - Membership Report";
        
        XLSX.writeFile(workbook, "caliber_fitness_membership_report.xlsx");
    };
                            ...
                            {loading ? (
                                ...
                                filteredMemberships.map(membership => (
                                    <tr key={membership._id} className="border-b hover:bg-gray-50">
                                        <td className="py-3 px-4">{membership.gymMemberId?.first_name} {membership.gymMemberId?.last_name}</td>
                                        <td className="py-3 px-4">{membership.subscriptionId?.name}</td>
                                        <td className="py-3 px-4">{membership.startDate ? new Date(membership.startDate).toLocaleDateString() : "N/A"}</td>
                                        <td className="py-3 px-4">{membership.endDate ? new Date(membership.endDate).toLocaleDateString() : "N/A"}</td>
